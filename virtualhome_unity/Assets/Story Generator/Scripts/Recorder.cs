...
namespace StoryGenerator.Recording
{
    ...
    public class Recorder : MonoBehaviour
    {
        ...
        void SaveRenderedFromCam(string pathPrefix, int camPassNo, bool isOpticalFlow, int cam_id=0)
        {
            ...
            Texture2D snapshot = new Texture2D(ImageWidth, ImageHeight, TextureFormat.RGB24, false);
            cam.Render();
            RenderTexture.active = cam.targetTexture;



            int resWidth = renderRT.width;
            int resHeight = renderRT.height;
            ...
            // Debug.Log("Saving image " + frameNum + " " + ImageSynthesis.PASSNAMES[camPassNo] + " " + cam_id);
            ...
            if (camPassNo == ImageSynthesis.PASS_NUM_DEPTH) {
                filePath += ".exr";
                bytes = snapshot.EncodeToEXR(Texture2D.EXRFlags.CompressZIP);
            } else {
                filePath += ".png";
